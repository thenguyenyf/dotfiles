...
- **Two install paths**, each chosen for a different reason:
  1. **Symlinked into the bash-completion user dir** (`~/.local/share/bash-completion/completions/`): `.git-completion.bash` → `git` and `.west-completion.bash` → `west`. The image's default `.bashrc` sources `bash_completion`, which lazy-loads this directory by command name, so the scripts are picked up without this repo shipping its own `.bashrc`. Entries are named `git`/`west` (not their dotfile names) because that's what the loader matches; the `git` entry deliberately shadows the system `/usr/share/bash-completion/completions/git`.
...
- `install.sh` — entry point, run by `coder dotfiles` on every workspace start. Must stay **idempotent** (safe to run repeatedly, which it is on every workspace launch). Uses `set -euo pipefail`.
- `.git-completion.bash`, `.west-completion.bash` — vendored upstream completion scripts (Git's core completion, GPL-2.0; west's completion). Treat as third-party; don't hand-edit. `install.sh` symlinks them into `~/.local/share/bash-completion/completions/` as `git` and `west`.
...
- To add a new completion script, add a `name:file` pair to the `for spec in ...` loop in `install.sh`; it gets symlinked into the bash-completion user dir under `name`. The script must register completion when sourced (e.g. end in `complete -F ... <name>`).
